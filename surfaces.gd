...
static func window_facade(wall: Color, glow: Color, lit: float = 1.6, cols: int = 4, rows: int = 4) -> StandardMaterial3D:
	...
	m.emission_enabled = true
	m.emission_texture = tex            # the same bright-window mask drives emission -> windows glow, wall doesn't
...
# Generate a window-grid tile: a dark wall with a grid of brighter window cells (with mullion gaps). Used as
# both albedo and emission so the windows glow and the wall doesn't.
